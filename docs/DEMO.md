...
> arithmetic over scraped prices. 72 of the 128 dish prices in the dataset are null.
> Routing has been running on data that is 56% blind — and nothing was watching. No
> data contract, no metric, no alert."
...
**Wording discipline:** what makes this claim true is that the preview is drawn from
`dispatch.json` at load time — same code, different dispatch, different picture. Say
"generated from the dispatch", not "generated by AI", and be ready to prove it by
changing the branch numbers on camera.
